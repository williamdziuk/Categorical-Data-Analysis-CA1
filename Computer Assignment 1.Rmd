...
Pearson's $\chi^2$
statistic:$X^2 = \sum_i\sum_j\frac{(O_{ij}-E_{ij})^2}{E_{ij}} = N\sum_{ij}(p_{i+}p_{+j}\bigg( \frac{(O_{ij}/N) - p_{i+}p_{+j}}{p_{i+}p_{+j}} \bigg)^2$
Where - $p_i+ = \sum_j \frac{O_{i,j}}{N}$ -
$p_+j = \sum_i \frac{O_{i,j}}{N}$ - $O_{i,j}$ is the observed cell
count. - $N$ is the total sample size.

Likelihood Ratio Test
$G^2 = -2\ln \frac{\text{sup}_{H_a}l(p_0)}{\text{sup}_{H_0}l(p_1, p_2)} = -2(n_{+1}\ln(\frac{n_{+1}}{N}) + n_{+2}\ln(\frac{n_{+2}}{N})) + 2(\sum_{ij} n_{ij} \ln(\frac{n_{ij}}{N}))$
...
We are performing the Pearsons test and Likelihood ratio test to compare
the probability of admission for men vs women. Our null hypothesis is
$H_0: p_{1,1} = p_{2,1}$ and our alternative Hypothesis is
...
The odds of men being admitted is 1.84 that of women. The $95\%$
confidence interval $[1.62, 2.09]$.
...
From this we see that the probability of men being admitted is $1.26\%$
times as high as for women, with the confidence interval being
$[1.20, 1.31]$.
...
From this, we see that the odds ratio is 1.05, but the p-value for the
null hypothesis: $H_0: p_{1,1} = p_{1,2}$ is zero, indicating that the
effect of gender on the opinion is statistically significant.
...
In a case like this, it's good to have the odds ratio, along with the
confidence interval, which we can see is $[1.017, 1.77]$. This tells the
ratio is indeed very close to 1, but not exactly 1. while if we had only
looked at the p-value, we might assume that the odds ratio is not close
to 1.
